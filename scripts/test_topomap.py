...
def plot_topomap(data, pos, axes=None, outline=None):
    """
    绘制topomap图。
    ...
    # --- 网格范围必须匹配轮廓范围 ---
    grid_x_min, grid_x_max = np.min(x_outline), np.max(x_outline)
    grid_y_min, grid_y_max = np.min(y_outline), np.max(y_outline)

    # 1. 缩放电极位置
    # (这个 4,6.5 是 "magic number"，您需要确保这个缩放比例是正确的，
    #  它决定了电极在大脑轮廓内的相对大小)
    normalized_pos = np.stack((config.POSITION[:,0] * (grid_x_max / 4), 
                               config.POSITION[:,1] * (grid_y_max / 6.5)), axis=1)
    ...
    # (推荐) 绘制电极点，以检查对齐情况
    ax.scatter(normalized_pos[:, 0], normalized_pos[:, 1], c='white', s=25, zorder=10, label='电极位置',edgecolors='black')
...
def main():
    ...
    for ax_i_index, (condition, value) in zip(range(4), tfr_all_conditions.items()):
        ...
        for ax_j_index, t_index in enumerate(np.arange(-3, 4)):
            ...
            if condition == 'rest':
                ...
                times_points = np.where(times == time)[0][0]
            else:
                time = times[t_index * config.TMAP_TIME_GAP]     # 非休息状态时间点从3开始
                times_points = np.where(times == time)[0][0]
            # 绘制topomap
            plot_topomap(tfr[:, times_points], pos = config.POSITION, axes=ax, outline=(x_outline, y_outline))
            ax.set_title(f'{condition} {time:.2f}')
    ...
    fig.savefig(os.path.join(output_dir, 'topomap_all_conditions_mi.png'))
